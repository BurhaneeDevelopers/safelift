...
import { fetchWithRevalidation } from "@/utils/sanity";
...
async function getCategories() {
  const result = await fetchWithRevalidation(`*[_type == "mainCategory"]`, {}, 60);
...
const ProductCards = ({ title, categoryImage, slug }) => {
  return (
    <Link
      className="relative block !size-96 m-3"
      href={`/products/${slug.current}`}
      data-aos="fade-up"
    >
      <span className="absolute inset-0 border-4 border-gray-200 shadow" />
